...
CREATE TABLE `STUDY_GROUP` (
  ...
  CONSTRAINT check_sgdatesanity CHECK (`SgCreated` <= `SgUpdated` AND `SgUpdated` <= CURRENT_TIMESTAMP),
);
...
CREATE TABLE `TARGET` (
  ...
  PRIMARY KEY (`SgUrl`, `EventNum`)
  CONSTRAINT check_targetsanity CHECK (`TargetSetDate` <= CURRENT_TIMESTAMP),
);
